...
void VideoChannel::video_player() {
    ...
    SwsContext *sws_ctx = sws_getContext(pContext->width, pContext->height,
                                         pContext->pix_fmt, // 原始图形信息相关的 （输入）
    ...
    while (isPlaying) {
         ...
         * sws_scale函数执行完成后， （rgba）dst_data, dst_linesize---》显示到屏幕上
         * */
